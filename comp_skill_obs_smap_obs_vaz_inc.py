...
for macro in macros_inc:
    ...
    for basin in bas_new:
        ...
        vazpaste = load_flow_obs(basin_fullname, 1)
    ...
    fig.suptitle(u'Índices: CORRELAÇÃO, MBE e RMSE \n Obs_smap x Vazpast / {0} - 1981_2010 \n Bacia: {1} - Usinas:'
                 u' Incrementais'.format(month.capitalize(), macro_name.capitalize()), fontsize=250,
                 fontweight='bold')
    ...
    path_out = ('/home/leidinice/documentos/results/indices_flow/inc/81-10/{0}/{1}/'.format(month, macro_name))
    if not os.path.exists(path_out):
        create_path(path_out)

    plt.savefig(os.path.join(path_out, '{0}_inc_indices_{1}_{2}.png'.format(macro_name, param, month)), dpi=25)
    plt.close('all')
    plt.cla()

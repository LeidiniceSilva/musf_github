...
for mon in range(1, 12 + 1):
    ...
    for macro in macros:
        ...
        for basin in bas_new3:
            ...
            model_data = import_model_data(model, mon, basin)
            obs_data   = import_obs_data(mon, basin)
        ...
        fig.suptitle(u'Índices: NS, MBE e RMSE \n {0} x OBS - {1} / Previsão: {2} - {3}_{4} (Sextas) \n Bacia: {5}'
                     .format(model.upper(), type.upper(), month.capitalize(), ihind, fhind, macro.capitalize()),
        ...
        path_out = ("/home/leidinice/documentos/results/{0}/{1}/skill/{2}/{3}/{4}/{5}/"
                    .format(model, type, scale, param, month,  macro))

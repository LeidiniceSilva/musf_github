...
def import_model_data(model_name, target_date, basin):
    ...
    file_modelname = "{0}/{1}_{2}_{3}_fcst_{4}_{5}_{6}_{7}_{8}." \
                     "nc".format(dir_modelname, param, scale, model_name, start_rundate, star_fcstdate,
                                 end_fcstdate, type, basin_fullname)
    try:
...
for macro in macros:
    ...
    for basin in basins:
        ...
        for mon in range(1, 12 + 1):
            month = datetime(2009, mon, 01).strftime("%b").lower()
    ...
    dir_file  = "/home/leidinice/documentos/results/gfs05/{0}/extremes_values/".format(type)
    file_name = '{0}extremes_values_0916_{1}.asc'.format(dir_file, macro)

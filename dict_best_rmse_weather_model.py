...
from hidropy.utils.hidropy_utils import date2index, create_path, lsname
...
def define_dates(target_date):
    ...
    str_mon = target_ifcstdate.strftime("%m")
...
def import_hind_model_data(model_name, mon, basin):
    ...
    for year in range(1981, 2016 + 1):
        ...
        dir_modelname   =  "{0}/io/{1}/hind8110/{2}/{3}/{4}_thiessen/{5}/".format(HIDROPY_DIR, model_name, month, time_freq, var_name, macro)
        file_modelname  = "{0}_{1}_{2}_hind8110_fcst_{3}_{4}_{5}_thiessen_{6}.nc".format(var_name, time_freq, model_name, start_rundate, start_fcstdate, end_fcstdate, basin)
...
def import_hind_obs_data(model_name, mon, basin):
	...
	inc_cal = Dataset(file_name_cal)
        time_cal = inc_cal.variables['time']
        aux_cal = inc_cal.variables[var_name][:]

    	# operation period 
	file_name_ope = '{0}/io/inmet_ana_merge/operation/{1}/' \
			'{2}_thiessen/{3}/{2}_{1}_{4}_obs_{5}_thiessen_{6}.nc' \
			.format(HIDROPY_DIR, time_freq, var_name, macro, data_base_ope,
			ope_obs_period, basin)
	
	linkf = lsname(file_name_ope)
	inc_ope = Dataset(linkf)
        time_ope = inc_ope.variables['time']
	aux_ope = inc_ope.variables[var_name][:]
	
        aux_tot = np.concatenate((aux_cal, aux_ope), axis=0)
	
        ifcstdate = datetime.strptime(start_fcstdate, "%Y%m%d")
        ffcstdate = datetime.strptime(end_fcstdate, "%Y%m%d")

        iidx = date2index(datetime(ifcstdate.year, ifcstdate.month, ifcstdate.day), time_cal)
        fidx = date2index(datetime(ffcstdate.year, ffcstdate.month, ffcstdate.day), time_ope)

        aux = aux_tot.variables[var_name][iidx:fidx+1]
        obs.append(aux)

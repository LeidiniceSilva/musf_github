...
if __name__ == '__main__':
    ...
    for macro in macros_total:
        ...
        for basin in bas_new:
            ...
            arq1  = "{0}/{1}_{2}_bases_obs_19810101_20170401_smap_{3}.nc".format(link1, param, scale, basin_fullname)
            ...
            stc_obs_acc.append(st_obs2[0::12] + st_obs2[1::12] + st_obs2[3::12])
            ...
            link2 = home + "/io/{0}/smap_{1}/{2}/{3}/{4}".format(param, scale, model, month, macro_name)
            ...
            link3 = home + "/io/{0}/smap_{1}/{2}/{3}/{4}".format(param, scale, model, month, macro_name)
            ...
            flow_corrected_gumbel1 = gumbel_correction(np.squeeze(ste_hind1), np.squeeze(stc_obs1),
                                                       np.squeeze(ste_fcst1))
            flow_corrected_gumbel2 = gumbel_correction(np.squeeze(ste_hind2), np.squeeze(stc_obs2),
                                                       np.squeeze(ste_fcst2))
            flow_corrected_gumbel3 = gumbel_correction(np.squeeze(ste_hind3), np.squeeze(stc_obs3),
                                                       np.squeeze(ste_fcst3))

            flow_corrected_gamma_desag = gamma_desag_correction(np.squeeze(ste_hind_acc), np.squeeze(stc_obs_acc),
                                                                np.squeeze(ste_fcst_acc))
            flow_corrected_gamma_desag1.append((np.squeeze(ste_fcst1) / np.squeeze(ste_fcst_acc)) *
                                               flow_corrected_gamma_desag)
            flow_corrected_gamma_desag2.append((np.squeeze(ste_fcst2) / np.squeeze(ste_fcst_acc)) *
                                               flow_corrected_gamma_desag)
            flow_corrected_gamma_desag3.append((np.squeeze(ste_fcst3) / np.squeeze(ste_fcst_acc)) *
                                               flow_corrected_gamma_desag)
            ...
            flow_corrected_gamma = np.array([rmse_gamma1, rmse_gamma2, rmse_gamma3])
            GAMMA.append(flow_corrected_gamma)
            ...
            flow_corrected_gumbel = np.array([rmse_gumbel1, rmse_gumbel2, rmse_gumbel3])
            GUMBEL.append(flow_corrected_gumbel)
            ...
            flow_corrected_gamma_desag2 = np.array([rmse_gamma_desag1, rmse_gamma_desag2, rmse_gamma_desag3])
            GAMMA_DESAG.append(flow_corrected_gamma_desag2)
        ...
        for i in range(len(bas_new)):
            ...
            fig.suptitle(u'Correções: GAMMA, GUMBEL e GAMMA DESAG. \n Modelo: {0} / Previsão: {1} - 2011_2016 \n Bacia:'
                         u' {2} - Usinas: Totais'.format(model.upper(), mon.capitalize(), macro_name.capitalize()),
        ...
        path_out = ("/home/leidinice/documentos/results/{0}/correc_rmse/{1}/{2}/{3}_smap/{4}/"

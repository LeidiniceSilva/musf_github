...
def gumbel_correction(hind, clim_obs, fcst):
    ...
    for ii in fcst:
        if ii == 0.0:
            corrected_fcst_gumbel.append(np.copy(ii))
        else:
...
if __name__ == '__main__':
    ...
    for macro in macros_inc:
        ...
        for basin in bas_new:
            ...
            st1 = variable1[240:600]  # 1981-2010
            st2 = variable1[252:603]  # 1982-2011
            ...
            for year2 in range(2011, 2017):
                ...
                new_year1   = start_date1 + relativedelta(months=2)
        ...
        for i in range(len(bas_new)):
            ...
            fig.suptitle(u'Correções: GAMMA,'
                         u' GUMBEL e GAMMA DESAG. Modelo: {0} / Previsão: {1} - 2011_2016 - Bacia: {2} - Usinas:'
                         u' Incrementais'.format(model.upper(), month.capitalize(), macro_name.capitalize()),
        ...
        path_out = ('/home/leidinice/documentos/results/{0}/correc_rmse/{1}/{2}/{3}_thiessen/{4}/'
                    'incrementais/'.format(model, mon, scale, param, macro_name))
